...
class PopupScreenProcessing{
    ...
    constructor(){
        ...
        this.create_text_type = "Two";
        
        this.separator_text_size = 20;
    ...
    show(){
        ...
        //top_button_frame.style.backgroundColor = "blue";
        ...
        //text_frame.style.backgroundColor = "yellow";
        ...
        //bottom_button_frame.style.backgroundColor = "red";
    ...
    CreateCrossMark(elem){
        ...
        // ボタン作成クラスがインスタンス化されていない場合は処理を抜ける
        //if(this.ButtonProcessing == null){console.log("[注意] : ButtonProcessingクラスが存在していないため後続処理を実行できません\n[場所] : <クラス> PopupScreenProcessing <メソッド> CreateCrossMark");return;};
        ...
        this.ButtonProcessing.set_font_size             (this.Cross_font_size        );
        this.ButtonProcessing.set_font_color            (this.Cross_font_color       );
        this.ButtonProcessing.set_background_color      (this.Cross_background_color );
        this.ButtonProcessing.set_width                 (this.Cross_width            );
        this.ButtonProcessing.set_height                (this.Cross_height           );
    ...
    CreateOKButton(elem){
        ...
        this.ButtonProcessing.set_font_size         (this.font_size          );
        this.ButtonProcessing.set_background_color  (this.OK_background_color);
        this.ButtonProcessing.set_font_color        (this.OK_font_color      );
        this.ButtonProcessing.set_width             (this.default_width      );
        this.ButtonProcessing.set_height            (this.default_height     );
    ...
    CreateOKCancelButton(elem){
        ...
        // キャンセルボタン作成
        this.ButtonProcessing.set_background_color      (this.cancel_background_color);
        this.ButtonProcessing.set_font_color            (this.cancel_font_color      );
    ...
    CreateTextCancelButton(elem){
        ...
        // テキストボタン作成
        this.ButtonProcessing.set_parent_elem         (button_frame                       );
        this.ButtonProcessing.set_background_color    (this.text_background_color         );
        this.ButtonProcessing.set_font_size           (this.font_size                     );
        this.ButtonProcessing.set_font_color          (this.text_font_color               );
        this.ButtonProcessing.set_width               (text_width                         );
        this.ButtonProcessing.set_height              (this.default_height                );
    ...
    TwoColumnsText = (elem)  => {
        ...
        let one_column_width = (parseInt(elem.style.width) - colon_width) / 2;
        ...
        if(this.parentheses){
            ...
            //parentheses_text_width = text_max_width;
        ...
        for(let row in popup_text){
            ...
            for(let linefeed = 0;linefeed < linefeed_num;linefeed++){
                ...
                // 右側のテキスト
                // -------------------+
                if(popup_text[row][0][linefeed] != undefined){
                ...
                // 左側のテキスト
                // -------------------+
                if(popup_text[row][1][linefeed] != undefined){
    ...
    GettingTextMaxWidth = (text,column) => {
        ...
        for(let row in text){
            ...
            max = max > dummy.getBoundingClientRect()["width"] ? max : dummy.getBoundingClientRect()["width"];
    ...
    CalculatePopupTextHeight = (text) => {
        ...
        for(let row in text){
            ...
            for(let column in text[row]){
                ...
                if(size[column] == undefined){size[column] = 0;};
                
                for(let linefeed in text[row][column]){
                    
                    dummy.innerText      = text[row][column][linefeed]["text"];
                    dummy.style.fontSize = text[row][column][linefeed]["size"];
